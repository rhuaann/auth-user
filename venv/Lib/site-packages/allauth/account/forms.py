...
class LoginForm(forms.Form):
    ...
    remember = forms.BooleanField(label=_("Lembre de mim"), required=False)
        ...
        if app_settings.SIGNUP_EMAIL_ENTER_TWICE:
            self.fields["email2"] = forms.EmailField(
                ...
                widget=forms.TextInput(
                    attrs={
                        "type": "email",
                        "placeholder": _("Confirmação de endereço de e-mail"),
                    }
                ),
            )
        if email_required:
...
class ChangePasswordForm(PasswordVerificationMixin, UserForm):
    oldpassword = PasswordField(
        label=_("Senha Antiga"), autocomplete="current-password"
    )
    password1 = SetPasswordField(
        label=_("Nova Senha"),
        help_text=password_validation.password_validators_help_text_html(),
    )
    password2 = PasswordField(label=_("Confirmar Nova Senha"))
...
class SetPasswordForm(PasswordVerificationMixin, UserForm):
    ...
    password2 = PasswordField(label=_("Confirmar Senha"))
...
class ResetPasswordKeyForm(PasswordVerificationMixin, forms.Form):
    password1 = SetPasswordField(label=_("Nova Senha"))
    password2 = PasswordField(label=_("Confirmar Senha"))
